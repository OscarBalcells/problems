...
int main() { 
    ...
    ll cost = 0; // min cost to cover all until i
    
    // (pos[j] - pos[i]) ^ 2 + cost
    // = pos[j]^2 + pos[i]^2 - 2*pos[i]*pos[j] + cost
    // = (-2*pos[j]) * (pos[i]) + (pos[j]^2 + cost) + (pos[i]^2)
    // ->    m       *    x     +         b         + (part of x, added on time)  
    
    for(int i = 0; i < n; ++i) {
        ds.addLine(-(-ll(2)*pos[i]), -(ll(pos[i])*ll(pos[i])+cost+c)); // -m and -b
        cost = -(ds.query(pos[i]))+ll(pos[i])*ll(pos[i]);
    }
